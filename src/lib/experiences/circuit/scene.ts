...
export async function setup(ctx: SetupContext): Promise<CircuitState> {
	...
	const defaultSpeed = moveSpeedParam ? (moveSpeedParam.default as number) : 100;
	...
	const world = new CircuitWorld(ctx.scene, player.rig.position, initialColor, initialDensity);
