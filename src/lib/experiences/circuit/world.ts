...
// Basic setup
// const cityManager = new CyberpunkCityManager(scene, playerPosition, staticStations);
// cityManager.init();
...
// Runtime controls
// cityManager.setNeonIntensity(2.0);
...
class CircuitChunk {
  ...
  generate(material: THREE.Material, density: number) {
    ...
    for (let a = 0; a < numAnchors; a++) {
      const ax =
        Math.round(
          ((hash(this.worldX, this.worldZ, a * 5) - 0.5) * CHUNK_SIZE) /
            GRID_SIZE,
        ) * GRID_SIZE;
      const az =
        Math.round(
          ((hash(this.worldX, this.worldZ, a * 7) - 0.5) * CHUNK_SIZE) /
            GRID_SIZE,
        ) * GRID_SIZE;
    ...
    for (let i = 0; i < numMainTraces; i++) {
      const anchorIndex = Math.floor(
        hash(this.worldX, this.worldZ, i * 3) * anchors.length,
      );
      ...
      const traceLength =
        6 + Math.floor(hash(this.worldX, this.worldZ, i * 17) * 12);
      ...
      const cellPhase =
        hash(this.worldX + currentPos.x, this.worldZ + currentPos.z, i) *
        Math.PI *
        2;
      ...
      for (let step = 0; step < traceLength; step++) {
        ...
        if (
          Math.abs(nextPos.x) > CHUNK_SIZE / 2 ||
          Math.abs(nextPos.z) > CHUNK_SIZE / 2
        )
          break;
        ...
        if (step === traceLength - 1) {
          ...
          if (minDist < CHUNK_SIZE * 0.2) {
            ...
            nextPos = currentPos
              ...
              .addScaledVector(
                currentDir,
                currentPos.distanceTo(closestAnchor),
              );
          }
        }
        ...
        for (let b = 0; b < numParallelLines; b++) {
          ...
          if (
            step === traceLength - 1 ||
            (step < traceLength - 1 &&
              hash(currentPos.x, currentPos.z, step * 3) < 0.25)
          ) {
            tryAddVia(p2);
          }
        }
        ...
        if (
          ...
          const turn =
            hash(currentPos.x, currentPos.z, step * 7) > 0.5 ? 1 : -1;
    ...
    if (segmentTransforms.length > 0) {
      ...
      this.segmentsMesh = new THREE.InstancedMesh(
        segGeo,
        material,
...
export class CircuitWorld {
  ...
  private customShaderUniforms: { [key: string]: THREE.IUniform } = {
    uTime: { value: 0.0 },
    uColor: { value: new THREE.Color("#00ff33") },
  };
  ...
  constructor(
    ...
    this.customShaderUniforms.uColor.value.set(initialColor);
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
                ...
                void main() {
                    ...
                    float wave = sin(uTime + fragPhase);

                    // Definieren der drei Farbstufen für den Circuit-Fluss
                    vec3 color1 = vec3(0.05, 0.45, 0.1); // Dunkelgrün (Basis)
                    vec3 color2 = vec3(0.0, 0.6, 0.9);   // Blau (Aktiv)
                    vec3 color3 = uColor;                // Custom Color (Peak)

                    // Weiche Übergänge statt harter Sprünge (verhindert Flackern)
                    float t1 = smoothstep(0.0, 0.4, wave);
                    float t2 = smoothstep(0.6, 0.9, wave);
  ...
  update(playerPosition: THREE.Vector3, delta: number) {
    // Tatsächliche Bewegung seit dem letzten Frame berechnen
    const movement = this.lastUpdatePosition.distanceTo(playerPosition);
    this.lastUpdatePosition.copy(playerPosition);

    const safeDelta = delta > 0.1 ? 0.016 : delta;

    // Geschwindigkeit als Basis für die Animation (gedeckelt gegen Aliasing)
    const speed = movement / (safeDelta + 0.0001);
    const speedMultiplier = Math.min(10.0, 1.0 + speed * 0.02);
  ...
  applySettings(id: string, value: any) {
    if (id === "traceDensity") {
      ...
      this.chunks.clear();
      this.updateChunks();
    } else if (id === "traceColor") {
      // Farbe des Peaks direkt im Shader aktualisieren
      if (this.customShaderUniforms.uColor.value instanceof THREE.Color) {
